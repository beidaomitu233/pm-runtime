...
import type { DiagramStatus, DiagramSummary, DiagramType } from '../types/diagramContracts'
import './diagrams.css'
...
function DiagramRow({ diagram }: { diagram: DiagramSummary }) {
  ...
  return <article className="diagram-row"><div className={`diagram-type ${diagram.diagramType}`}>{diagram.diagramType === 'flowchart' ? '流程' : '泳道'}</div><div className="diagram-main"><Link className="diagram-title" to={`/projects/${diagram.projectId}/diagrams/${diagram.id}`}>{diagram.title}</Link><div className="diagram-meta"><span>{diagram.orientation === 'horizontal' ? '横向' : '纵向'}</span><span>revision {diagram.currentRevisionNo || '—'}</span>{diagram.meetingId && <span>来源会议：{diagram.meetingId}</span>}</div>{diagram.warnings?.length ? <p className="diagram-warning">有 {diagram.warnings.length} 条 warning</p> : null}</div><div className="diagram-actions"><span className={`diagram-status ${diagram.status}`}>{diagramStatusLabel(diagram.status)}</span>{editable ? <Link className="button sm" to={`/projects/${diagram.projectId}/diagrams/${diagram.id}`}>查看详情</Link> : <button className="button sm" disabled title="当前图形不可编辑">查看详情</button>}</div></article>
}
