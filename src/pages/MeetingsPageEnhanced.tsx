...
import type { MeetingImportStatus, MeetingSourceType, MeetingSummary } from '../types/meetingContracts'
import './meetings.css'

export function MeetingsPageEnhanced() {
  const { projectId } = useParams()
  const [query, setQuery] = useState('')
  const [status, setStatus] = useState<MeetingImportStatus | ''>('')
  ...
  if (meetings.isPending) return <section className="meeting-page"><div className="skeleton-list"><div /><div /><div /></div></section>
  if (meetings.isError) return <section className="meeting-page"><PageError error={meetings.error} onRetry={() => void meetings.refetch()} /></section>
  return <section className="meeting-page"><div className="page-heading"><div><p className="eyebrow">项目主线</p><h1>会议</h1><p>导入会议内容后，宿主 Agent 可按 meeting ID 读取正文并生成流程图。</p></div><button className="button primary" onClick={() => setShowPaste(true)}>粘贴会议</button></div><div className="meeting-toolbar"><label className="search-field"><span aria-hidden="true">⌕</span><span className="sr-only">搜索会议</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索会议标题" /></label><select aria-label="状态筛选" value={status} onChange={(event) => setStatus(event.target.value as MeetingImportStatus | '')}><option value="">全部状态</option><option value="ready">可读取</option><option value="importing">导入中</option><option value="failed">导入失败</option></select><select aria-label="来源筛选" value={sourceType} onChange={(event) => setSourceType(event.target.value as MeetingSourceType | '')}><option value="">全部来源</option><option value="paste">粘贴</option><option value="txt">TXT</option><option value="md">MD</option><option value="docx">DOCX</option></select></div>{meetings.data.items.length ? <div className="meeting-list">{meetings.data.items.map((meeting) => <MeetingRow key={meeting.id} meeting={meeting} />)}</div> : <div className="empty-state"><div className="empty-icon">＋</div><h2>还没有会议资料</h2><p>先粘贴一份会议记录，生成可追溯的 meeting ID。</p><button className="button primary" onClick={() => setShowPaste(true)}>粘贴会议</button></div>}{showPaste && <PasteMeetingDialog pending={createMeeting.isPending} error={createMeeting.error} onClose={() => { setShowPaste(false); createMeeting.reset() }} onSubmit={(title, text) => createMeeting.mutate({ title, text })} />}</section>
}

function MeetingRow({ meeting }: { meeting: MeetingSummary }) {
  return <article className="meeting-row"><div className={`meeting-source ${meeting.sourceType}`}>{meeting.sourceType.toUpperCase()}</div><div className="meeting-main"><Link to={`/projects/${meeting.projectId}/meetings/${meeting.id}`} className="meeting-title">{meeting.title}</Link><div className="meeting-meta"><span>{meeting.charCount.toLocaleString('zh-CN')} 字符</span><span>{formatDate(meeting.createdAt)}</span>{meeting.originalFilename && <span title={meeting.originalFilename}>{meeting.originalFilename}</span>}</div>{meeting.importStatus === 'failed' && <p className="meeting-error">{meeting.importErrorMessage || meeting.importErrorCode || '导入失败'}</p>}</div><span className={`meeting-status ${meeting.importStatus}`}>{statusLabel(meeting.importStatus)}</span></article>
}
...
function statusLabel(status: MeetingImportStatus) { return status === 'ready' ? '可读取' : status === 'importing' ? '导入中' : '导入失败' }
