...
| COM-046 | 架构 | 接口 | 当前 `src/types/contracts.ts` 与规划中的 `packages/contracts` 形成两套合同来源。（原 v2 侧 COM-020，合并时重编号） | 统一运行时 Schema 与 TS 类型来源，避免前后端字段漂移。 | src/api、src/types | packages/contracts、HTTP/MCP | 无 | 执行中 | `packages/contracts` 为唯一合同源；TASK-001 删除或改造前端重复合同，所有调用方从共享包消费。 |
