...
| COM-019 | 架构 | 工程 | 旧规划建议迁移到 `apps/desktop`，但当前 React 代码已经稳定存在根目录 `src/`。 | v0.1 避免纯目录重构，按现状收口模块边界。 | 根目录 React | runtime-sidecar | 无 | 已确认 | v0.1 保持根目录 `src/` 作为 Desktop UI；新增 `src-tauri/` 承载桌面壳，v0.1 后再评估目录迁移。 |
| COM-020 | 架构 | 接口 | 当前 `src/types/contracts.ts` 与规划中的 `packages/contracts` 形成两套合同来源。 | 统一运行时 Schema 与 TS 类型来源，避免前后端字段漂移。 | src/api、src/types | packages/contracts、HTTP/MCP | 无 | 已确认 | `packages/contracts` 为唯一合同源；TASK-001 删除或改造前端重复合同，所有调用方从共享包消费。 |
| COM-021 | 架构 | 工程 | 历史 FRONTEND/BACKEND/DATABASE 任务拆分使完整功能必须后期集中联调。 | 采用纵向 full-stack TASK，一项任务覆盖页面、API、业务、DB 和验收。 | 全部页面 | 全部服务 | 全部表 | 已确认 | 新任务来源为 `TASK_BOARD.md` 和 `docs/tasks/`；旧 FE/BE 计划保留历史参考但停止派单。 |
